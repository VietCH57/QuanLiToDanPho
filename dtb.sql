...
CREATE DATABASE IF NOT EXISTS QLToDanPho 
    CHARACTER SET utf8mb4 
    COLLATE utf8mb4_unicode_ci;

USE QLToDanPho;

-- Drop tables theo thứ tự ngược (child trước, parent sau)
SET FOREIGN_KEY_CHECKS = 0;

DROP TABLE IF EXISTS users;
DROP TABLE IF EXISTS COMMENDATION_NOTIFICATION;
DROP TABLE IF EXISTS COMMENDATION_DECISION;
DROP TABLE IF EXISTS COMMENDATION_PROPOSAL;
DROP TABLE IF EXISTS AWARD;
DROP TABLE IF EXISTS TEMPORARY_ABSENCE;
DROP TABLE IF EXISTS TEMPORARY_RESIDENCE;
DROP TABLE IF EXISTS CITIZENSHIP_HISTORY;
DROP TABLE IF EXISTS HOUSEHOLD_BOOK;
...
CREATE TABLE CITIZEN (
    ...
    Management_Unit_ID VARCHAR(10), 
    Date_of_Death DATE,
    FOREIGN KEY (Management_Unit_ID) REFERENCES ADMINISTRATIVE_UNIT(Unit_ID)
);

-- 3. Table HOUSEHOLD_BOOK 
CREATE TABLE HOUSEHOLD_BOOK (
    Household_Book_ID VARCHAR(50) PRIMARY KEY,
    Head_of_Household_CCCD_ID VARCHAR(20) NOT NULL,
    Permanent_Residence_Address VARCHAR(200) NOT NULL,
    Issue_Date DATE NOT NULL,
    Number_of_Members INTEGER NOT NULL,
    Issuing_Unit_ID VARCHAR(10), 
    CONSTRAINT unique_head_of_household UNIQUE (Head_of_Household_CCCD_ID),
    FOREIGN KEY (Head_of_Household_CCCD_ID) REFERENCES CITIZEN(CCCD_ID),
    FOREIGN KEY (Issuing_Unit_ID) REFERENCES ADMINISTRATIVE_UNIT(Unit_ID)
);

-- 4. Update link between CITIZEN and HOUSEHOLD_BOOK
ALTER TABLE CITIZEN ADD COLUMN Household_Book_ID VARCHAR(50);
ALTER TABLE CITIZEN ADD CONSTRAINT fk_citizen_household FOREIGN KEY (Household_Book_ID) REFERENCES HOUSEHOLD_BOOK(Household_Book_ID);

-- 5. Table TEMPORARY_RESIDENCE
CREATE TABLE TEMPORARY_RESIDENCE (
    Temp_Residence_ID VARCHAR(50) PRIMARY KEY,
    CCCD_ID VARCHAR(20) NOT NULL,
    Temporary_Address VARCHAR(200) NOT NULL,
    Start_Date DATE NOT NULL,
    End_Date DATE,
    Registration_Unit_ID VARCHAR(10),
    FOREIGN KEY (CCCD_ID) REFERENCES CITIZEN(CCCD_ID),
    FOREIGN KEY (Registration_Unit_ID) REFERENCES ADMINISTRATIVE_UNIT(Unit_ID)
);
...
CREATE TABLE CITIZENSHIP_HISTORY (
    ...
    Performing_Unit_ID VARCHAR(10),
    FOREIGN KEY (CCCD_ID) REFERENCES CITIZEN(CCCD_ID),
    FOREIGN KEY (Performing_Unit_ID) REFERENCES ADMINISTRATIVE_UNIT(Unit_ID)
);
...
CREATE TABLE COMMENDATION_DECISION (
    ...
    Note TEXT,
    FOREIGN KEY (Proposal_ID) REFERENCES COMMENDATION_PROPOSAL(Proposal_ID)
);
...
CREATE TABLE users (
    ...
    Working_Unit_ID VARCHAR(10), -- Added: User's working unit
    ...
    CONSTRAINT fk_users_unit FOREIGN KEY (Working_Unit_ID) REFERENCES ADMINISTRATIVE_UNIT(Unit_ID)
);
...
INSERT INTO HOUSEHOLD_BOOK (Household_Book_ID, Head_of_Household_CCCD_ID, Permanent_Residence_Address, Issue_Date, Number_of_Members, Issuing_Unit_ID)
VALUES ('HK_ADMIN', '000000000001', 'Hanoi', '2005-05-05', 1, 'DV001');
